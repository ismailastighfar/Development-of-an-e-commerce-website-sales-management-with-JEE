...
<div class="row  mt-5 ml-5 p-5 position-relative col-8 justify-content-end ">
  ...
  <div class="col-sm-3">
    <select class="form-select" id="specificSizeSelect">
      <option selected>codeCmd</option>
      <option >codeArt</option>
        <option>client</option>
        <option>qteCmd</option>
      <option>dateCmd</option>
    </select>
  </div>
</div>
...
<table class="table table-hover table-bordered border-secondary ">
  <thead class="text-light">
    <tr  class="table-primary">
      <th scope="col">codeCmd</th>
      <th scope="col">codeArt</th>
      <th scope="col">client</th>
      <th scope="col">qteCmd</th>
  ...
  <tbody >
   <s:iterator value="cmdList">
    <tr>
      ...
      <s:url namespace="/" action="deleteCmd" var="delete">
	  <s:param name="codecmd">
	    <s:property value="codeCmd"/>
	  </s:param>
	</s:url>
	<td><s:a href="%{delete}" class="btn btn-danger delete-btn" >cancel</s:a></td>
    </tr>
     </s:iterator>
  </tbody>
</table>
...
$(document).ready(function() {
	  $('#specificSizeInputGroupUsername').on('keyup', function() {
	    ...
	    var column = $('#specificSizeSelect').val() === 'codeCmd' ? 0 : $('#specificSizeSelect').val() === 'codeArt' ? 1 : $('#specificSizeSelect').val() === 'client' ? 2 :$('#specificSizeSelect').val() === 'qteCmd' ? 3:4 ;
